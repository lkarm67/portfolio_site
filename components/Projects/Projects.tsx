        ...
        {projects.map((p, i) => (
          <motion.li key={i} variants={item} className={css.card}>
            <div className={css.imageWrapper}>
              ...
              <div className={css.overlay}>
                <a href={p.live} target="_blank">{t.projects.live}</a>
                {Array.isArray(p.github) ? (
                  p.github.map((g: { label: string; link: string }, i: number) => (
                    <a key={i} href={g.link} target="_blank">
                      {g.label}
                    </a>
                  ))
                ) : (
                  <a href={p.github} target="_blank">{t.projects.github}</a>
                )}
              </div>
            </div>
            ...
            <div className={css.content}>
              ...
              <p>{t.projects.items[p.key]}</p>
              <p>
                <span className={css.span}>{t.projects.type}: </span>{t.projects[p.type]}
              </p>
              <p>
                <span className={css.span}>{t.projects.role}: </span>
                  {p.role}
